...
const AddArticleModal = ({ show, handleClose, handleSave }) => {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            const response = await axios.post('/Article/Article', JSON.stringify(addArticleViewModel), {
                headers: {
            ...
            console.log(data);
            if (data.articleWasAdded) {
                handleSave(addArticleViewModel.Title, data.articleId);
                handleClose();
            } else {
                setErrorMessage('Failed to add article');
            }
        } catch (error) {
            setErrorMessage('Failed to add article. Try again later!');
        }
    };
                ...
                <div className="modal-content">
                    ...
                    <div className="modal-body">
                        <form onSubmit={handleSubmit}>
                            ...
                            <button type="submit" className="btn btn-primary w-100/">Add Article</button>
                        </form>
                    </div>
                </div>
            </div>
